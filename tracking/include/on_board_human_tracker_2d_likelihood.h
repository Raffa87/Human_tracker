...
#define t_LIKELIHOOD_SIGMA      150              // in mm std of the 2D Gaussian for creation likelihood
...
//Function to update the sliding window containing past clusters
void update_cluster_sliding_widow(ClusterStruct cl, ClusterStruct *sliding_window, int size, int *index, int *count){

    #if 1==0

    memcpy((char*) (sliding_window + *index), (char*)&cl, sizeof(ClusterStruct));

    #else

    sliding_window[*index].numClusters = cl.numClusters;
    //copy the clusters
    for (int k=0; k < sliding_window[*index].numClusters; k++){
        sliding_window[*index].cluster[k].numPoints     = cl.cluster[k].numPoints;
        sliding_window[*index].cluster[k].semi_major    = cl.cluster[k].semi_major;
        sliding_window[*index].cluster[k].semi_minor    = cl.cluster[k].semi_minor;
        sliding_window[*index].cluster[k].flag          = cl.cluster[k].flag;
        sliding_window[*index].cluster[k].centroid.x = cl.cluster[k].centroid.x;
        sliding_window[*index].cluster[k].centroid.y = cl.cluster[k].centroid.y;
  
        for (int i=0; i < sliding_window[*index].cluster[k].numPoints; i++){
            sliding_window[*index].cluster[k].point[i].x = cl.cluster[k].point[i].x ;
            sliding_window[*index].cluster[k].point[i].y = cl.cluster[k].point[i].y ;
        }
    }

    #endif

    //update the sliding widow index
    (*index)++;
    if (*index > size - 1)
        *index = 0;
    //update the count
...
//Function to compute the current cluster likelihood using the sliding window containing past clusters
void compute_sliding_widow_likelihood(ClusterStruct *sliding_window, int size, int index, int count, double forgetting_factor, double *cluster_acc_likelihood){

    int past_index;
    double x_cl, y_cl, x_pcl, y_pcl, cl_sigma;
    
    //Set Gaussian 
    cl_sigma = t_LIKELIHOOD_SIGMA;

    //Get the index of last cluster window
    int current_index = index - 1;
    if(current_index < 0)
        current_index = current_index + size;
    
    //Loop on the current clusters
    for(int i = 0; i < sliding_window[current_index].numClusters; i++){
        
        //reset the likelihood of that cluster
        cluster_acc_likelihood[i] = 0.0;

        //Check the flag for not associated clusters
        if(sliding_window[current_index].cluster[i].flag == true){
            //this is a current cluster not associated to a tracked human
            x_cl = sliding_window[current_index].cluster[i].centroid.x; 
            y_cl = sliding_window[current_index].cluster[i].centroid.y;    
            
            //loop on current and past clusters windows
            for( int j = 0; j < count; j++){
            
                //get the past cluster window
                past_index = current_index - j;
                if(past_index < 0)
                    past_index = past_index + size;

                //loop on the past clusters
                for(int k = 0; k < sliding_window[past_index].numClusters; k++){

                    //Check past cluster flag
                    if(sliding_window[past_index].cluster[k].flag == true){
                        //this is a past cluster not associated to a tracked human
                        x_pcl = sliding_window[past_index].cluster[k].centroid.x; 
                        y_pcl = sliding_window[past_index].cluster[k].centroid.y;    

                        //accumulate likelihood
                        cluster_acc_likelihood[i] += pow(forgetting_factor, k) * gauss(x_cl, y_cl, x_pcl, y_pcl, cl_sigma, cl_sigma);
                    }
                }
            }
        }
...
//Birth of humans 
//Creates a human based on the likelihood 
//Using only present clusters.  Sets cluster.flag to false
void t_Create_Humans_likelihood(ClusterStruct *cl, double *cluster_acc_likelihood, double likelihood_threshold, t_Entity *human, int *t_Max_ID_Index,  int t_People_ID[ssm_MAX_NUM_OF_PEOPLE] ){
  double mindist = 1000000;
  int id = -1;
  ...
  for( int i=0; i<cl->numClusters; i++ ) {
    ...
    if( id!=-1 && mindist<=t_MAX_STEP_SIZE && cluster_acc_likelihood[i] >= likelihood_threshold && cluster_acc_likelihood[id] >= likelihood_threshold){
